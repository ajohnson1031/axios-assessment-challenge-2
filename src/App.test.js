...
describe("BookSearch", () => {
  const books = JSON.parse(readFileSync("./src/sample_data/books.json"));
  let wrapper;

  beforeEach(() => {
    wrapper = mount(<BookSearch books={JSON.parse(JSON.stringify(books))} />);
  });
  afterEach(() => wrapper.unmount());

  describe("the correct elements were rendered", () => {
    it("should have 5 input elements for required search fields", () =>
      [".author", ".title", ".language", ".country", ".year"].forEach((e) =>
        expect(wrapper.exists(e)).toBe(true)
      ));
  ...
  describe("searching by each field alone", () => {
    describe("modifying the author field", () => {
      ...
      it("should filter books on the author query", () => {
        ...
        expect(
          wrapper.find(".book").hostNodes().at(0).html().includes("Dalloway")
        ).toBe(true);
        expect(
          wrapper.find(".book").hostNodes().at(1).html().includes("Lighthouse")
        ).toBe(true);
      });
    });
    ...
    describe("modifying the title field", () => {
      ...
      it("should filter books on the author query", () => {
        ...
        expect(
          wrapper.find(".book").hostNodes().at(0).html().includes("Euripides")
        ).toBe(true);
        expect(
          wrapper
            .find(".book")
        ...
        expect(
          wrapper
            ...
            .at(2)
            .html()
            .includes("The Death of Ivan Ilyich")
        ).toBe(true);
      });
    });
    ...
    describe("modifying the country field", () => {
      ...
      it("should filter books on the country query", () => {
        ...
        [
          ...
          expect(
            wrapper.find(".book").hostNodes().at(i).html().includes(e)
          ).toBe(true)
        );
      });
    });
    ...
    describe("filtering on 192_ and english", () => {
      beforeEach(() => {
        wrapper
          .find(".year")
          .hostNodes()
          .simulate("change", { target: { value: "192" } });
        wrapper
          .find(".language")
          .hostNodes()
    ...
    describe("filtering on all fields with trim", () => {
      beforeEach(() => {
        ...
        wrapper
          .find(".country")
          .hostNodes()
          .simulate("change", { target: { value: "ni" } });
        wrapper
          .find(".language")
          .hostNodes()
          .simulate("change", { target: { value: "english" } });
        wrapper
          .find(".year")
          .hostNodes()
          .simulate("change", { target: { value: "195" } });
      });
      ...
      it("should filter books on all fields", () => {
        ...
        [
          ...
          expect(
            wrapper.find(".book").hostNodes().at(0).html().includes("")
          ).toBe(true)
        );
      });
    });
    ...
    describe("filtering on terms that have no results", () => {
      beforeEach(() => {
        wrapper
          .find(".year")
          .hostNodes()
          .simulate("change", { target: { value: "190" } });
        wrapper
          .find(".language")
          .hostNodes()
